...
const Home = () => {
    ...
    useEffect(() => {
        axios.get('http://backend:5000/get')
            .then(result => setTodos(result.data))
            .catch(err => console.log(err));
    }, []);

    const edit = (id) => {
        axios.put(`http://backend:5000/edit/${id}`)
            .then(result => {
    ...
    const Update = (id, updatedTask) => {
        axios.put(`http://backend:5000/update/${id}`, { task: updatedTask })
            .then(result => {
                console.log(result.data);
                const updatedTodos = todos.map(todo => {
                    if (todo._id === id) {
    ...
    const Hdelete = (id) => {
        axios.delete(`http://backend:5000/delete/${id}`)
            .then(result => {
                console.log(result.data);
